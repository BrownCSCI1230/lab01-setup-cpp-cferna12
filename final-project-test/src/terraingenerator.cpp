...
glm::vec3 TerrainGenerator::getPosition(int row, int col) {
    ...
    float x = 3.0 * row / m_resolution;
    float y = 3.0 * col / m_resolution;
...
//        height+= (1.f/i)*computePerlin(x*pow(freq,i), y *pow(freq,i))/2;
//    }
    float z = computePerlin(x * freq, y * freq) / 2;
    float z1 = 0.5*computePerlin(x * freq, y * freq) / 2;
    float z2 = 0.25*computePerlin(x * 8, y * 8) / 2;
    float z3 = 0.125*computePerlin(x * 16, y * 16) / 2;
    float z4 = 0.0625*computePerlin(x * 32, y * 32) / 2;
    float z5 = 0.03125*computePerlin(x * 64, y * 64) / 2;
    // Task 7: combine multiple different octaves of noise to produce fractal perlin noise

    // Return 0 as placeholder
    return z+z1+z2+z3;+z4+z5;
//    return height;
}
...
//    }
    ...
    if(glm::dot(up_normal, normal)/(pow(glm::length(up_normal),2)) >= 0.7 || position[2] >= 0.2){
          return glm::vec3(1,1,1);
    }

    // Return white as placeholders
...
float TerrainGenerator::computePerlin(float x, float y) {
    ...
    std::vector<glm::vec2> grid_indices;

    glm::vec2 top_left = {(int)x, (int)y}; //A
    glm::vec2 top_right = {top_left[0]+1, top_left[1]}; //B
    glm::vec2 bot_left = {top_left[0], top_left[1]+1}; //C
    glm::vec2 bot_right= {top_left[0]+1, top_left[1]+1}; //D
    grid_indices = {top_left, top_right, bot_left, bot_right};

    // Task 2: compute offset vectors
    std::vector<glm::vec2> offsets;
    glm::vec2 pos = {x,y};
    offsets = {pos - top_left, pos- top_right, pos - bot_left, pos- bot_right};

    // Task 3: compute the dot product between offset and grid vectors
    std::vector<float> dots;
    for(int i = 0; i < offsets.size(); i++){
        glm::vec2 rand_vec = sampleRandomVector(grid_indices[i][1], grid_indices[i][0]); //rand vec for grid indices
        dots.push_back(glm::dot(rand_vec, offsets[i]));
    }


    // Task 5: use your interpolation function to produce the final value
    float dx = x- top_left[0];
    float dy = y - top_left[1];
    float G = interpolate(dots[0], dots[1], dx);
    float H = interpolate(dots[2], dots[3], dx);
